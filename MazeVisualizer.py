...
class MazeVisualizer():
    ...
    def __init__(self, maze: MazeContract, solution_nodes=set()):
    ...
    def get_char_on_type(cell):
        if cell == Maze_Types.Wall:
            ...
            return MazeVisualizer.get_ascii_block(chr(0x2588)*2, 'red')
        elif cell == Maze_Types.Start or cell == Maze_Types.End:
            return MazeVisualizer.get_ascii_block(chr(0x2588)*2, 'green')
        else:
            return '  '

    def __str__(self):

        return '\n'.join([''.join([MazeVisualizer.get_char_on_type(cell) for cell in row]) for row in self.content])